public class Merge2SortedList {
    ...
    public static ListNode mergeTwoLists(ListNode list1, ListNode list2) {
        ...
        if (list1 == null && list2 == null ) { ret = list1; return ret;}
        if (list1 == null && list2 != null) {ret = list2; return ret;}
        if (list1 != null && list2 == null) {ret = list1; return ret;}
        int intLastValue =0;
        while (list1 != null) {
            while (list2 != null) {
                if (list1.val > list2.val) {
                    if ((list1.val <= ret.val) || (ret.val == 0)) {
                        if (ret.val == 0 ) {
                            ...
                            if (list1.next == null) { ret = addToList(ret, list1.val);}
                            intLastValue = list2.val;
                        } else {
                            ret = addToList(ret, list2.val);
                            if (list1.next == null) { ret = addToList(ret, list1.val);}
                            intLastValue = list2.val;
                        }
                    } else if (list2.val > intLastValue){
                        ret = addToList(ret, list2.val);
                    }
    ...
    public static void main(String[] args) {
        ListNode node1 = new ListNode(1);//new ListNode(1, new ListNode(2, new ListNode(4)));
